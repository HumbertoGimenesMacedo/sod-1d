...
% estipula o método
for METHOD = [HARTEN IMP_HARTEN]
    % ordem do método
    for order = [1 2]
        % razão entre as pressões em ambos os lados do diafragma
        for pressureRatio = [5 10 20 50 100]
            scheme_name = "";
            switch (METHOD)
                case LAX_WENDROFF
                ...
                case IMP_HARTEN
                    if order == 1
                        scheme_name = "First Order Implicit Harten Scheme";
                    elseif order == 2
                        scheme_name = "Second Order Implicit Harten Scheme";
                    end
            end

            fprintf('Executando o esquema %s de %d ordem de precisão para a razão %d ...\n', scheme_name, order, pressureRatio);
            ...
            % gera um PDF com as saídas
            generateImages(METHOD, scheme_name, order, pressureRatio, gamma);
